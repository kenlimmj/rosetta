...
export default class ThematicBreakDetector implements IProcessor<BlockNode, ThematicBreakNode> {
  process(node: BlockNode): ThematicBreakNode | null {
    ...
    // Perform a quick check: There can be an indent of at most three spaces, followed by
    // a _, -, or *, in order for the thematic break to be valid.
    ...
    if (breakChar === '_' || breakChar === '-' || breakChar === '*') {
      ...
      const tokens = nodeContent.trim().split('').filter(t => t.trim().length > 0);
